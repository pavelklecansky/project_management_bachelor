...
public class OutcomeController {
    ...
    @NonNull
    Converter<UUID, OutcomeCategoryCommand> uuidOutcomeCategoryCommandConverter;
    ...
    public ResponseEntity<?> getOutcomes(@PathVariable UUID id) {
        ...
        return ResponseEntity.ok(projectCommand.getPhases().stream()
                .map(PhaseCommand::getOutcomes)
                .flatMap(List::stream)
                .map(outcomeMapper::outcomeCommandToOutcomeResponse)
    ...
    public ResponseEntity<?> getOutcomesByCategory(@PathVariable UUID id) {
        ...
        return ResponseEntity.ok(outcomeCategoryCommand.getOutcomes().stream()
                .map(outcomeMapper::outcomeCommandToOutcomeResponse)
                .toList());
    }
    ...
    public ResponseEntity<SuccessResponse> createOutcome(@RequestBody OutcomeRequest request) {
        ...
        if (request.getOutcomeCategory() != null) {
            outcomeCommand.setOutcomeCategory(
                    uuidOutcomeCategoryCommandConverter.convert(request.getOutcomeCategory()));
        }
        ...
        return ResponseEntity.created(location)
                .body(SuccessResponse.builder()
                        ...
                        .data(outcomeMapper.outcomeCommandToOutcomeResponse(createOutcome))
                        .build());
    }
    ...
    public ResponseEntity<SuccessResponse> updateOutcome(@PathVariable UUID id, @RequestBody OutcomeRequest request) {
        ...
        return ResponseEntity.ok(SuccessResponse.builder()
                ...
                .data(outcomeMapper.outcomeCommandToOutcomeResponse(update))
                .build());
    }
}
