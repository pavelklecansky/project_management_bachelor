...
public class GroupController {
    ...
    public ResponseEntity<?> getGroups() {
        List<GroupResponse> groupResponses = groupService.getGroups().stream()
                .map(groupMapper::groupCommandToGroupResponse)
                .toList();
    ...
    public ResponseEntity<?> getGroup(@PathVariable UUID id) throws NoSuchElementFoundException {
        return groupService
                .getGroup(id)
                .map(organizationCommand ->
                        ResponseEntity.ok(groupMapper.groupCommandToGroupResponse(organizationCommand)))
    ...
    public ResponseEntity<SuccessResponse> createGroup(@RequestBody GroupRequest request) {
        ...
        groupCommand.setMembers(request.getMembers().stream()
                .map(groupMemberCommand -> {
        ...
        return ResponseEntity.created(location)
                .body(SuccessResponse.builder()
                        ...
                        .data(groupMapper.groupCommandToGroupResponse(createdGroup))
                        .build());
    }
    ...
    @PutMapping(path = "{id}")
    @PreAuthorize("hasAnyRole('ADMIN','SUPER_ADMIN')")
    public ResponseEntity<SuccessResponse<GroupResponse>> updateGroup(
            @PathVariable UUID id, @RequestBody GroupRequest request) {
        GroupCommand groupCommand = groupMapper.groupRequestToGroupCommand(request);
        groupCommand.setMembers(request.getMembers().stream()
                .map(groupMemberCommand -> {
                    GroupMemberCommand memberCommand = new GroupMemberCommand();
                    memberCommand.setId(
                            Objects.isNull(groupMemberCommand.getId())
                                    ? UUID.randomUUID()
                                    : groupMemberCommand.getId());
                    memberCommand.setPosition(groupMemberCommand.getPosition());
                    memberCommand.setUser(UUIDToUserCommandConverter.convert(groupMemberCommand.getUser()));
                    return memberCommand;
                })
        ...
        return ResponseEntity.ok(SuccessResponse.<GroupResponse>builder()
                .message("Group was successfully updated.")
                .data(groupMapper.groupCommandToGroupResponse(update))
                .build());
    }
}
