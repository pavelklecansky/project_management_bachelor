...
public class ResultController {
    ...
    @NonNull Converter<UUID, OutcomeCommand> uuidOutcomeCommandConverter;
    ...
    public ResponseEntity<?> getResultOfOutcome(@PathVariable UUID id) {
        ...
        return ResponseEntity.ok(outcomeCommand.getResults().stream().map(resultMapper::resultCommandToResultResponse).collect(Collectors.toSet()));
    }
    ...
    public ResponseEntity<SuccessResponse> createResult(@RequestBody ResultRequest request) {
        ...
        URI location = ServletUriComponentsBuilder
                .fromCurrentContextPath().path("/results/{id}")
                .buildAndExpand(createdResult.getId()).toUri();
        return ResponseEntity.created(location).body(SuccessResponse.builder().message("Result was successfully created.").data(resultMapper.resultCommandToResultResponse(createdResult)).build());
    }
    ...
    public ResponseEntity<SuccessResponse> deleteResult(@PathVariable UUID id) {
        ...
        return ResponseEntity.ok(SuccessResponse.builder().message("Result was successfully deleted.").build());
    }
    ...
    public ResponseEntity<SuccessResponse> updateResult(@PathVariable UUID id, @RequestBody ResultRequest request) {
        ...
        return ResponseEntity.ok(SuccessResponse.builder().message("Result was successfully updated.").data(resultMapper.resultCommandToResultResponse(update)).build());
    }
}
