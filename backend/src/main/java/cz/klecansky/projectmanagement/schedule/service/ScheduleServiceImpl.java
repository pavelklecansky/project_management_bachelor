...
public class ScheduleServiceImpl implements ScheduleService {
    ...
    @NonNull TaskScheduleMapper taskScheduleMapper;
    @NonNull TaskScheduleRepository taskScheduleRepository;
    @NonNull RowRepository rowRepository;

    @Override
    public Optional<ScheduleCommand> getByProject(UUID id) throws NoSuchElementFoundException {
        return scheduleRepository.findAll().stream().filter(scheduleEntity -> scheduleEntity.getProject().getId().equals(id)).map(scheduleMapper::scheduleEntityToScheduleCommand).findFirst();
    }
    ...
    public ScheduleCommand createTask(UUID id, TaskCommand taskRequestToTaskCommand) {
        ...
        Integer maxInt = scheduleEntity.getTasks().stream().max(Comparator.comparing(TaskScheduleEntity::getId)).map(rowEntity -> rowEntity.getId())
                .orElse(0);
        ...
        taskScheduleEntity.setRow(scheduleEntity.getRows().stream().filter(rowEntity -> rowEntity.getId() == taskScheduleEntity.getResourceId()).findFirst().orElseThrow(NoSuchElementFoundException::new));
    ...
    public ScheduleCommand createRow(UUID id, RowCommand rowRequestToRowCommand) {
        ...
        ScheduleEntity scheduleEntity = scheduleRepository.findById(id).orElseThrow(NoSuchElementFoundException::new);
        Integer maxInt = scheduleEntity.getRows().stream().max(Comparator.comparing(RowEntity::getId)).map(rowEntity -> rowEntity.getId())
                .orElse(0);
    ...
    public TaskCommand updateTask(UUID id, TaskCommand taskRequestToTaskCommand) {
        TaskScheduleEntity taskScheduleEntity = taskScheduleRepository.findById(id).orElseThrow(NoSuchElementFoundException::new);
    ...
    public List<RowCommand> getRows(UUID id) {
        ScheduleEntity scheduleEntity = scheduleRepository.findById(id).orElseThrow(NoSuchElementFoundException::new);
        return scheduleEntity.getRows().stream().map(rowMapper::rowEntityToRowCommand).toList();
    }
