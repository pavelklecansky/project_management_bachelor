...
public class TaskUtils {
    ...
    private static boolean isAssigned(TaskCommand taskCommand, UserPrincipal userPrincipal) {
        return !Objects.isNull(taskCommand.getAssigned()) && taskCommand.getAssigned().getId().equals(userPrincipal.getUser().getId());
    }

    private static boolean isAssignedForGroup(TaskCommand taskCommand, UserPrincipal userPrincipal) {
        return !Objects.isNull(taskCommand.getAssignedForGroup()) && taskCommand.getAssignedForGroup().getMembers().stream().anyMatch(memberCommand -> memberCommand.getUser().getId().equals(userPrincipal.getUser().getId()));
    }
}
