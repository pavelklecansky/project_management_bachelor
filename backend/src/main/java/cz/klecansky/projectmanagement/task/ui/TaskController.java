...
public class TaskController {
    ...
    @NonNull Converter<UUID, ProjectCommand> UUIDToProjectCommandConverter;
    @NonNull Converter<UUID, UserCommand> UUIDToUserCommandConverter;
    @NonNull Converter<UUID, GroupCommand> UUIDToGroupCommandConverter;
    ...
    public ResponseEntity<?> getTasks() {
        List<TaskResponse> taskResponse = taskService.getAll().stream().map(taskMapper::taskCommandToTaskResponse).toList();
        return ResponseEntity.ok(taskResponse);
    }

    @GetMapping("/usersTasks")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<?> getUsersTasks(@CurrentSecurityContext(expression = "authentication.principal") UserPrincipal userPrincipal) {
        List<TaskResponse> taskResponse = taskService.getAll().stream().filter(taskCommand -> TaskUtils.assignedToTask(taskCommand, userPrincipal)).map(taskMapper::taskCommandToTaskResponse).toList();
        return ResponseEntity.ok(taskResponse);
    }
    ...
    public ResponseEntity<SuccessResponse> createTask(@RequestBody TaskRequest request) {
        ...
        return ResponseEntity.created(location).body(SuccessResponse.builder().message("Task was created successfully.").data(taskMapper.taskCommandToTaskResponse(createdProject)).build());
    }
    ...
    public ResponseEntity<SuccessResponse> deleteTask(@PathVariable UUID id) {
        ...
        return ResponseEntity.ok(SuccessResponse.builder().message("Task was successfully deleted.").build());
    }
    ...
    public ResponseEntity<SuccessResponse> updateTask(@PathVariable UUID id, @RequestBody TaskRequest request) {
        ...
        return ResponseEntity.ok(SuccessResponse.builder().message("Task was successfully updated.").data(taskMapper.taskCommandToTaskResponse(update)).build());
    }
    ...
    public ResponseEntity<SuccessResponse> addComment(@PathVariable UUID id, @Valid @RequestBody CommentRequest request, @CurrentSecurityContext(expression = "authentication.principal") UserPrincipal userPrincipal) {
        ...
        return ResponseEntity.ok(SuccessResponse.builder().message("Comment was successfully added.").data(taskMapper.taskCommandToTaskResponse(updatedProject)).build());
    }

    @DeleteMapping(path = "{id}/comment/{idComment}")
    @PreAuthorize("hasAnyRole('ADMIN','SUPER_ADMIN')")
    public ResponseEntity<SuccessResponse> deleteComment(@PathVariable UUID id, @PathVariable UUID idComment, @CurrentSecurityContext(expression = "authentication.principal") UserPrincipal userPrincipal) {
        TaskCommand updatedProject = taskService.deleteComment(id, idComment);
        return ResponseEntity.ok(SuccessResponse.builder().message("Comment was successfully deleted.").data(taskMapper.taskCommandToTaskResponse(updatedProject)).build());
    }
