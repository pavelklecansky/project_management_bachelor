...
public class BudgetController {
    ...
    public ResponseEntity<?> getCategory(@PathVariable UUID id) throws NoSuchElementFoundException {
        return budgetService
                .getCategory(id)
                .map(category ->
                        ResponseEntity.ok(budgetCategoryMapper.budgetCategoryCommandToBudgetCategoryResponse(category)))
                .orElseThrow(NoSuchElementFoundException::new);
    }
    ...
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<SuccessResponse> createBudgetCategory(
            @PathVariable UUID id, @RequestBody BudgetCategoryRequest request) {
        BudgetCategoryCommand createBudgetCategory = budgetService.createBudgetCategory(
                id, budgetCategoryMapper.budgetCategoryRequestToBudgetCategoryCommand(request));
        return ResponseEntity.ok(SuccessResponse.builder()
                .message("Budget category was successfully created.")
                .data(budgetCategoryMapper.budgetCategoryCommandToBudgetCategoryResponse(createBudgetCategory))
                .build());
    }

    @PostMapping(path = "item/{id}")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<SuccessResponse> createBudgetItem(
            @PathVariable UUID id, @RequestBody BudgetItemRequest request) {
        BudgetItemCommand createdBudgetItem =
                budgetService.createBudgetItem(id, budgetItemMapper.budgetItemRequestToBudgetItemCommand(request));
        return ResponseEntity.ok(SuccessResponse.builder()
                .message("Budget item was successfully created.")
                .data(budgetItemMapper.budgetItemCommandToBudgetItemResponse(createdBudgetItem))
                .build());
    }
    ...
    public ResponseEntity<SuccessResponse> updateItem(@PathVariable UUID id, @RequestBody BudgetItemRequest request) {
        BudgetItemCommand budgetItemCommand =
                budgetService.updateItem(id, budgetItemMapper.budgetItemRequestToBudgetItemCommand(request));
        return ResponseEntity.ok(SuccessResponse.builder()
                .message("Item was successfully updated.")
                .data(budgetItemMapper.budgetItemCommandToBudgetItemResponse(budgetItemCommand))
                .build());
    }
    ...
    public ResponseEntity<SuccessResponse> updateCategory(
            @PathVariable UUID id, @RequestBody BudgetCategoryRequest request) {
        BudgetCategoryCommand budgetCategoryCommand = budgetService.updateCategory(
                id, budgetCategoryMapper.budgetCategoryRequestToBudgetCategoryCommand(request));
        return ResponseEntity.ok(SuccessResponse.builder()
                .message("Category was successfully updated.")
                .data(budgetCategoryMapper.budgetCategoryCommandToBudgetCategoryResponse(budgetCategoryCommand))
                .build());
    }
