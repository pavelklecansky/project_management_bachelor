<script lang="ts">
	...
	const { form, handleSubmit } = createForm({
		...
		onSubmit: async (values) => {
			...
			const [createdSuccess, createdError] = await createPhase(id, transferify(values.phase));
			if (createdError) {
				error(createdError);
...
<ModalWindow outsideClickGotoPath={`/project/${id}/phases`} {handleSubmit}>
	<div class="px-4 mb-4">
		...
		<div class="w-full mt-4">
			...
			<DateInput
				title={'enter a start date for phases, the start cannot be before the start of the project'}
