<script lang="ts">
	...
	const { form, handleSubmit } = createForm({
		...
		onSubmit: async (values) => {
			...
			values.outcome.phase =
				Object.keys(values.outcome.phase).length === 0 ? null : values.outcome.phase;
			values.outcome.outcomeCategory =
				Object.keys(values.outcome.outcomeCategory).length === 0
					? null
					: values.outcome.outcomeCategory;
...
<ModalWindow outsideClickGotoPath={`./../${id}`} {handleSubmit}>
	<div class="px-4 mb-4">
		...
		<div class="w-full mt-4">
			...
			<DateInput
				title={'enter a start date for outcome, the start cannot be before the start of selected phase'}
