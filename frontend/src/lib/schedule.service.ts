...
export const createTask: (id: string, task: ScheduleTask) => Promise<[Schedule, any]> = async (
	id,
	task
) => {
	const [res, error] = await apiRequest({
...
export const updateTask: (id: string, task: ScheduleTask) => Promise<[ScheduleTask, any]> = async (
	id,
	task
) => {
	const [res, error] = await apiRequest({
