...
export const createBudgetCategory: (id: string, budgetCategory: BudgetCategory) => Promise<[BudgetCategory, any]> = async (id, budgetCategory) => {
    const [res, error] = await apiRequest({
...
export const createBudgetItem: (id: string, budgetItem: BudgetItem) => Promise<[BudgetItem, any]> = async (id, budgetItem) => {
    const [res, error] = await apiRequest({
...
export const updateCategory: (id: string, category: BudgetCategory) => Promise<[BudgetCategory, any]> = async (id, category) => {
    const [res, error] = await apiRequest({
        endpointName: `budget/category/${id}`,
        method: "PUT",
        body: category
...
export const updateItem: (id: string, item: BudgetItem) => Promise<[BudgetItem, any]> = async (id, item) => {
    const [res, error] = await apiRequest({
        endpointName: `budget/item/${id}`,
        method: "PUT",
        body: item
...
export const deleteBudgetCategory: (id: string) => Promise<[BudgetCategory | null, any]> = async (id) => {
    const endpointName = `budget/category/${id}`;
    const [res, error] = await apiRequest({
        endpointName,
        method: "DELETE",
