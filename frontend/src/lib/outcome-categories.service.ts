...
export const getOutcomeCategoriesOfProject: (
	id: string
) => Promise<[OutcomeCategory[], any]> = async (id) => {
...
export const createOutcomeCategory: (
	outcome: OutcomeCategory
) => Promise<[OutcomeCategory, any]> = async (outcome) => {
	const [res, error] = await apiRequest({
...
export const updateOutcomeCategory: (
	outcome: OutcomeCategory
) => Promise<[OutcomeCategory, any]> = async (outcome) => {
...
export const deleteOutcomeCategory: (id: string) => Promise<[OutcomeCategory | null, any]> = async (
	id
) => {
	const endpointName = `outcome-categories/${id}`;
	const [res, error] = await apiRequest({
