...
<script lang="ts">
    ...
    const renameFileModal = async (oldName: string) => {
        await open(EditFilePopup, {oldName, id, queryParams}, {}, {
            onClosed: async () => {
                await loadFilesInfo();
            }
        })
...
<div class="flex justify-between w-full mb-2">
    <div>
        {#if queryParams}
            <CreateButton text="🔼" on:click={() => {
                            $goto(`${$url()}?path=${removeLastFolderPath(queryParams)}`);
                        }}/>
        {/if}
    </div>

    <div>
        <CreateButton text="New folder" on:click={() => {
        ...
        {#each filesInfo as file}
            {#if file.folder}
                ...
                <div
                        ...
                        title={file.name}
                        on:click={() => {
                            $goto(`${$url()}?path=${queryParams+`/${file.name}`}`);
                        }}
                >
