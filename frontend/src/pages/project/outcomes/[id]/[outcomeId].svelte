        ...
        <div>
            ...
            <div class="flex justify-between gap-4">
                ...
                <div class="w-1/5 flex flex-col text-center">
                    ...
                    <a class="mt-2" href={$url(`./delete/${outcome.id}`)}
                    >
                        <DeleteButton text={"Delete outcome"} full={true}/>
                    </a
                    >
                    <a class="mt-2" href={$url(`./edit/${outcome.id}`)}
                    >
                        <EditButton text={"Edit outcome"} full={true}/>
                    </a
                    >
                    <a class="mt-2" href={$url(`./create-result/${outcome.id}`)}
                    >
            ...
            <div class="flex md:justify-start justify-center flex-wrap gap-5">
                {#each outcome.results as result (result.id)}
                    <div class="md:w-1/4 w-full max-w-sm px-4 py-3 bg-white rounded-md shadow-md flex-grow box-border">
                        ...
                        <div>
                            <a href={$url(`./${outcome.id}/result/${result.id}`)}
